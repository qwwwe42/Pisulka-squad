...
export const ShowCard: React.FC<ShowCardProps> = ({ show, onClick }) => {
  ...
  return (
    <button 
      type="button"
      onClick={onClick}
      aria-label={`Смотреть: ${show.title}`}
      className="group relative bg-bg-card rounded-3xl overflow-hidden border border-border-color shadow-soft card-glow-hover cursor-pointer flex flex-col h-full text-left w-full">
      ...
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-bg-app shrink-0">
        ...
        {/* Category Badge */}
        <div className="absolute top-2.5 left-2.5 px-2.5 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wider bg-bg-card/95 text-text-primary border border-border-color flex items-center gap-1.5 backdrop-blur-xs">
          {getCategoryIcon()}
        ...
        {(() => {
          ...
          return count > 0 ? (
            <div className="absolute top-2.5 right-2.5 px-2.5 py-1 rounded-lg text-[11px] font-bold bg-bg-card/95 text-yellow-500 border border-border-color flex items-center gap-1 backdrop-blur-xs font-mono">
              <span>★</span>
              <span>{average}</span>
            </div>
          ) : null;
        ...
        {/* Episode Count */}
        <div className="absolute bottom-2.5 right-2.5 px-2.5 py-1 rounded-lg text-[11px] font-bold bg-bg-card/95 text-accent-color border border-border-color backdrop-blur-xs">
      ...
      <div className="p-4.5 flex flex-col flex-1 justify-between gap-1.5">
        ...
        <div className="flex items-center justify-between mt-2 pt-2 border-t border-border-color text-[11px] text-text-muted">
          <span>{show.status === 'ongoing' ? 'Онгоинг' : 'Завершен'}</span>
          {show.status === 'ongoing' && show.nextEpisodeRelease && (
            <span className="text-accent-color font-bold">Таймер активен</span>
          )}
        </div>
      </div>
    </button>
